...
- **음성 안내(TTS)**: 확인 화면에서 *"○○님께 ○○원 보낼까요?"* 를 소리로 읽어줍니다(🔊 다시 듣기). 글자가 잘 안 보이는 어르신용 — 무료·온디바이스(브라우저 음성합성).

### 핵심 UX 원칙
**말로 "○○에게 ○○ 보내줘"는 항상 다음 단계로 진행됩니다.** 어르신을 막지 않는 게 핵심. 위험 신호만 보호자(아들)에게 알립니다.

| 상황 | 동작 |
|---|---|
| 등록된 분 · 평소 금액 | ✅ 바로 토스 송금화면으로 |
| 등록된 분 · 30만원 이상 | ⚠️ 큰아들에게 알려 확인 후 진행(직접 보내기도 가능) |
| **계좌를 말함**("류창한 신한은행 110…로 1원") | ✅ **막지 않고 토스로** → 토스가 성함·계좌 검증 → 🔔 큰아들 알림 → 보낸 뒤 **"저장" 누르면 목록 등록** |
| 이름만 있고 계좌 모름 | 🔔 큰아들에게 알려 확인(누군지·계좌) |

> **사전 등록 부담 해소.** 보호자가 떨어져 살아 일일이 등록하기 어려우므로, 어르신이 **계좌를 말하면 바로 토스로** 넘어갑니다. 토스 송금화면이 받는 사람 성함을 보여주니 그 자체가 1차 검증이고, 송금 후 **앱이 그 수취인을 기억**해 다음부턴 이름만 말하면 됩니다.
>
> ⚠️ **한계(정직)**: 토스는 닫힌 앱이라 ① 기존 송금 내역을 **불러올 수 없고** ② 송금 **완료를 자동 감지할 수 없습니다.** 그래서 저장은 사용자가 "보냈어요"를 누르는 방식(또는 돌아왔을 때 확인). 실서비스에선 오픈뱅킹으로 가면 이 한계가 사라집니다.

## 향후 방향 / 기존 데모 (참고)
- **[`OPENBANKING.md`](OPENBANKING.md)** — 오픈뱅킹 검토 기록. 실제 제품의 종착지(송금·완료확인·내역 자동등록 해결)이나 개인 신분으론 운영 불가하여 보류. 사업자/제휴 확보 시 재검토용 사전조사.
